(() => {
  ...
  function addStyles(){
    ...
    s.textContent=`
      .driver-brand-wrap{margin-top:12px;padding:15px;border-radius:18px;background:#0d1421;border:1px solid #263247}
      .driver-brand-title{font-size:14px;font-weight:900;margin-bottom:5px}
      .driver-brand-subtitle{font-size:10px;color:#7f8ca3;margin-bottom:11px}
      ...
      .driver-brand-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;max-height:330px;overflow:auto;padding-right:2px}
      .driver-brand{min-height:76px;border-radius:13px;background:#111a29;border:1px solid #243044;color:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;padding:7px 4px}
      .driver-brand.active{border-color:#facc15;background:rgba(250,204,21,.08);box-shadow:0 0 0 1px rgba(250,204,21,.15)}
      .driver-brand img{width:40px;height:34px;object-fit:contain}
      ...
      .driver-selected-brand{margin-top:9px;padding:9px 11px;border-radius:12px;background:rgba(34,197,94,.07);border:1px solid rgba(34,197,94,.16);font-size:11px;color:#a7b4c7}
  ...
  function render(){
    ...
    wrap.innerHTML=`<div class="driver-brand-title">🚗 Марка автомобиля</div>
      <div class="driver-brand-subtitle">Нажмите на логотип — марка автоматически заполнится в анкете</div>
